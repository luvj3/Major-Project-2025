...
def preprocess(data):
    ...
    # Add media_message column - check if a message contains '<Media omitted>'
    df['media_message'] = df['message'].apply(lambda x: 1 if '<Media omitted>' in x else 0)
    
    # Add url_count column - count URLs in each message
    df['url_count'] = df['message'].apply(lambda x: len(extract.find_urls(x)))
